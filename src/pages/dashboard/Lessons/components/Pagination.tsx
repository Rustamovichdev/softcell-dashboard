...
const baseClassName = "flex h-9 min-w-9 items-center justify-center rounded-lg px-2 text-sm transition disabled:cursor-not-allowed disabled:opacity-40";
    ...
    <nav aria-label="Sahifalash" className="mt-4 flex flex-wrap items-center justify-center gap-1">
      ...
      {pages.map((item) => (
        <button
          ...
          className={`${baseClassName} ${item === page ? "bg-gray-900 text-white" : "text-gray-600 hover:bg-gray-100"}`}
        >
          {item}
        </button>
      ))}
