...
type LessonModalProps = {
  lesson?: Partial<LessonFormValues>;
  onClose: () => void;
...
const LessonModal: FC<LessonModalProps> = ({ lesson, onClose, onSubmit }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label={lesson ? "Darsni tahrirlash" : "Dars qo'shish"}
      ...
      >
        ...
        <h2 className="text-lg font-semibold">{lesson ? "Darsni tahrirlash" : "Yangi dars qo'shish"}</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-5 flex flex-col gap-4">
          ...
          <div className="mt-1 flex gap-3">
            <button type="button" onClick={onClose} className="h-11 flex-1 rounded-lg border border-gray-200 text-sm font-medium text-gray-700 transition hover:bg-gray-50">
              Bekor qilish
            </button>
            <button type="submit" className="h-11 flex-1 rounded-lg bg-gray-900 text-sm font-medium text-white transition hover:bg-gray-800">
              Saqlash
            </button>
          </div>
        </form>
